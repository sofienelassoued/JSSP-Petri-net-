...
class PetriEnv(gym.Env):
  ...
  def step(self, action):
      ...
      for p in self.Places_obj:   #update internal clock 
         if p.enabled==True:
         ...
         for p in self.Places_obj: #Synchronising dic and Obj Places     
             self.Places_dict[p.pname]= [p.token,p.In_arcs, p.Out_arcs,p.process_time,p.features]
       
         for k in self.Places_obj: # initialise enabled status of place exept in propcess
      ...
      info.update({tuple(self.marking): self.Transition_names[action]})
      done=self.terminal
